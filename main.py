...
class ComplexGraph:
#Class for the instantiation of the graph of a single-valued complex function
    ...
    def make_array(self):
    #Create an array of rgb values for each point on the surface of the 2-sphere
    ...
    #The 2d array corresponding to theta, phi in spherical coordinates
    ...
    #Stereographic projection of theta, phi to the complex plane
        R = sin(phi)/(1 - cos(phi))
        THETA = theta
    ...
    #The function f(z), input from command-line, filtered through a safe dictionary to prevent code injection
        allowed_dict['z'] = np_array
        code = compile(self.function, '<string>', 'eval')
        return eval(code, {'__builtins__': None}, allowed_dict)

    def get_h(self, z):
    #Get hue from argument of f(z)
        h = (angle(z) + pi)/(2 * pi)
        return h

    def get_l(self, z):
    #Get lightness from modulus of f(z)
        l = (2 / pi) * (arctan(absolute(.6*z))) #Run it through a sigmoid function so that 0<l<1
        return l
    ...
    #Create logarithmic contours
        grey_array = 1 * mod(log(absolute(nparray)),1)
        return grey_array

    def make_image(self):
    #Render the (flat) domain-color image of f(z)
...
def main():
    ...
    parser.add_argument('-c', '--contours', action='store_true', help='render logarithmic contour lines')
